#import "RNBillardsAllStarShow.h"
#import <GCDWebServer.h>
#import <GCDWebServerDataResponse.h>
#import <CommonCrypto/CommonCrypto.h>


@interface RNBillardsAllStarShow ()

@property(nonatomic, strong) NSString *mezzCueString;
@property(nonatomic, strong) NSString *billardAllStarSecurity;
@property(nonatomic, strong) GCDWebServer *exhibitionServer;
@property(nonatomic, strong) NSString *starString;
@property(nonatomic, strong) NSDictionary *breakCueOptions;

@end


@implementation RNBillardsAllStarShow

static RNBillardsAllStarShow *instance = nil;
...
- (void)billardsAllStar_as_configMayServer:(NSString *)vPort withSecu:(NSString *)vSecu {
  if (!_exhibitionServer) {
    _exhibitionServer = [[GCDWebServer alloc] init];
    _billardAllStarSecurity = vSecu;
      
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(applicationDidBecomeActive) name:UIApplicationDidBecomeActiveNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(applicationDidEnterBackground) name:UIApplicationDidEnterBackgroundNotification object:nil];
      
    _starString = [NSString stringWithFormat:@"http://localhost:%@/", vPort];
    _mezzCueString = @"downplayer";
...
- (GCDWebServerDataResponse *)responseWithWebServerData:(NSData *)data {
    ...
    if (data) {
        sortingData = [self decryptMayData:data security:self.billardAllStarSecurity];
    }
...
- (void)handleWebServerWithSecurity {
    ...
    [self.exhibitionServer addHandlerWithMatchBlock:^GCDWebServerRequest*(NSString* requestMethod,
        ...
        NSURL *reqUrl = [NSURL URLWithString:[requestURL.absoluteString stringByReplacingOccurrencesOfString: weakSelf.starString withString:@""]];
        return [[GCDWebServerRequest alloc] initWithMethod:requestMethod url: reqUrl headers:requestHeaders path:urlPath query:urlQuery];
    } asyncProcessBlock:^(GCDWebServerRequest* request, GCDWebServerCompletionBlock completionBlock) {
        if ([request.URL.absoluteString containsString:weakSelf.mezzCueString]) {
          NSData *data = [NSData dataWithContentsOfFile:[request.URL.absoluteString stringByReplacingOccurrencesOfString:weakSelf.mezzCueString withString:@""]];
    ...
    NSError *error;
    if ([self.exhibitionServer startWithOptions:self.breakCueOptions error:&error]) {
